...
const BeautyHomePageSubmit = (values, mutate) => {
  ...
  values["content"]["about_us"]["services"]?.["banners"]?.forEach((elem, i) => {
    if (values[`serviceBannerImage${i}`]) {
      values["content"]["about_us"]["services"]["banners"][i]["image_url"] = values[`serviceBannerImage${i}`].hasOwnProperty('asset_url') ? values[`serviceBannerImage${i}`].asset_url : values[`serviceBannerImage${i}`].original_url;
    } else {
  ...
  if (values["aboutUsImage"]) {
    values["content"]["about_us"]["image_url"] = values["aboutUsImage"].hasOwnProperty('asset_url') ? values["aboutUsImage"].asset_url : values["aboutUsImage"].original_url;
  } else values["content"]["about_us"]["image_url"] = "";

  if (values["productImage"]) {
    values["content"]["product_video"]["image_url"] = values["productImage"].hasOwnProperty('asset_url') ? values["productImage"].asset_url : values["productImage"].original_url;
  } else values["content"]["product_video"]["image_url"] = "";

  if (values["productVideo"]) {
    values["content"]["product_video"]["video_url"] = values["productVideo"].hasOwnProperty('asset_url') ? values["productVideo"].asset_url : values["productVideo"].original_url;
