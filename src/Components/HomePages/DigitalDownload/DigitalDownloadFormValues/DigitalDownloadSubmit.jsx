...
const DigitalDownloadSubmit = (values, mutate) => {
  ...
  if (values["homeBannerBackgroundImage"]) {
    values["content"]["home_banner"]["background_image"] = values["homeBannerBackgroundImage"].hasOwnProperty('asset_url') ? values["homeBannerBackgroundImage"].asset_url : values["homeBannerBackgroundImage"].original_url;
  } else values["content"]["home_banner"]["background_image"] = "";
  if (values["homeBannerSubImage1"]) {
    values["content"]["home_banner"]["sub_image_1"] = values["homeBannerSubImage1"].hasOwnProperty('asset_url') ? values["homeBannerSubImage1"].asset_url : values["homeBannerSubImage1"].original_url;
  } else values["content"]["home_banner"]["sub_image_1"] = "";
  if (values["homeBannerSubImage2"]) {
    values["content"]["home_banner"]["sub_image_2"] = values["homeBannerSubImage2"].hasOwnProperty('asset_url') ? values["homeBannerSubImage2"].asset_url : values["homeBannerSubImage2"].original_url;
  } else values["content"]["home_banner"]["sub_image_2"] = "";

  if (values["productListMainImage"]) {
    values["content"]["products_list_2"]["image_url"] = values["productListMainImage"].hasOwnProperty('asset_url') ? values["productListMainImage"].asset_url : values["productListMainImage"].original_url;
