...
const ParallaxHomePageSubmit = (values, mutate) => {
  ...
  values["content"]["parallax_banner"]["banners"].forEach((elem, i) => {
    if (values[`parallaxBannerImage${i}`]) {
      values["content"]["parallax_banner"]["banners"][i]["image_url"] = values[`parallaxBannerImage${i}`].hasOwnProperty('asset_url') ? values[`parallaxBannerImage${i}`].asset_url : values[`parallaxBannerImage${i}`].original_url;
    } else {
