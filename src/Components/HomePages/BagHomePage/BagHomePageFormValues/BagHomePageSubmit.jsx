...
const BagHomePageSubmit = (values, mutate) => {
  ...
  values["content"]["grid_banner"]["banners"].forEach((elem, i) => {
    if (values[`gridBannerImage${i}`]) {
      values["content"]["grid_banner"]["banners"][i]["image_url"] = values[`gridBannerImage${i}`].hasOwnProperty('asset_url') ? values[`gridBannerImage${i}`].asset_url : values[`gridBannerImage${i}`].original_url;
    } else {
