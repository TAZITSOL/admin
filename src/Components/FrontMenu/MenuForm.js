...
const MenuForm = ({ mutate, updateId, loading,language }) => {
  ...
  const { data ,refetch:pageRefetch,isLoading:pageLoading } = useQuery([PagesAPI], () => request({ url: PagesAPI },router), {enabled:false, refetchOnWindowFocus: false,select: (res) =>res?.data?.data?.filter((elem) => elem?.status)?.map((elem) => {return { id: elem?.slug, name: elem?.title,slug:elem.slug };}),});
  useEffect(() => {
    updateId && refetch();
  }, [updateId]);
  useEffect(() => {
      ...
      {({ setFieldValue, values, errors }) => (
        <Form className="theme-form theme-form-2 mega-form">
          ...
          <Row>
            ...
            {values["link_type"] == "link" && ( <CheckBoxField name="is_target_blank" title="target_blank" />)}
