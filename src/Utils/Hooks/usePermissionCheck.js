...
const usePermissionCheck = (permissionTypeArr, keyToSearch) => {
  ...
  useEffect(() => {
    ...
    if (JSON.parse(localStorage.getItem("account_multikart"))?.permissions?.length > 0) {
      const securePaths = removeDuplicateObjects(ConvertPermissionArr(JSON.parse(localStorage.getItem("account_multikart"))?.permissions));
